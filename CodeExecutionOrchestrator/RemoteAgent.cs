using CodeExecutionOrchestrator.Agent;
        ...
        public void Run()
        {
            ...
            APIControllerIOC.NISOInvoked += async(sender, e) =>
            {
                if (_actionDictionary.ContainsKey(_transform(e.ActionName)))
                {
                    ...
                    {
                        APIControllerIOC.OnNISOCallback(e, new CodeExecutionOrchestrator.Core.InstanceDescriptor(_actionDictionary[_transform(e.ActionName)].ReturnType, result)); //Always return something, don't leave the server hanging
                    }
                }
            };
            ...
            APIControllerIOC.SISOInvoked += async(sender, e) =>
            {
                if (_actionDictionary.ContainsKey(_transform(e.ActionName)))
                {
                    ...
                    {
                        APIControllerIOC.OnSISOCallback(e, new CodeExecutionOrchestrator.Core.InstanceDescriptor(_actionDictionary[_transform(e.ActionName)].ReturnType, result)); //Always return something, don't leave the server hanging
                    }
                }
            };
        }
